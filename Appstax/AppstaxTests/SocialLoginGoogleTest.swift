...
@objc class SocialLoginGoogleTest: XCTestCase {
    ...
    override func setUp() {
        ...
        AXStubs.method("GET", urlPath: "/sessions/providers/google") { _ in
        ...
        OHHTTPStubs.stubRequestsPassingTest({
            request in
            return request.URL?.host == "accounts.google.com"
            }, withStubResponse: {
                request in
    ...
    func testShouldOpenGoogleAuthPage() {
        let async = expectationWithDescription("async")
        
        AXUser.login(provider: "google", fromViewController: nil) { _ in
            async.fulfill()
        }
        
        waitForExpectationsWithTimeout(timeout + 1) { error in
            AXAssertStringContains(self.requestedAuthUrl, needle: "accounts.google.com/o/oauth2/v2/auth")
        }
    }
    ...
    func testShouldUseClientIdFromProviderConfig() {
        ...
        clientId = "the-client-id-12345"
        AXUser.login(provider: "google", fromViewController: nil) { _ in
            async.fulfill()
        }
        
        waitForExpectationsWithTimeout(timeout) { error in
            AXAssertStringContains(self.requestedAuthUrl, needle: "client_id=the-client-id-12345")
        }
    }
    
    func testShouldAskForProfileScope() {
        let async = expectationWithDescription("async")
        ...
        waitForExpectationsWithTimeout(timeout) { error in
            AXAssertStringContains(self.requestedAuthUrl, needle: "scope=profile+email")
        }
    }
    
    func testShouldGenerateNonce() {
        let async = expectationWithDescription("async")
        ...
        waitForExpectationsWithTimeout(timeout) { error in
            AXAssertStringContains(self.requestedAuthUrl, needle: "nonce=")
        }
    }
    ...
    func testShouldSendAuthCodeAndRedirectUriFromDialogToServer() {
        ...
        redirectToUrl = "https://appstax.com/api/latest/sessions/auth?code=the-auth-code-23456"
        AXUser.login(provider: "google", fromViewController: nil) { _ in
            async.fulfill()
        }
    ...
    func testShouldReturnUserAndSessionWhenLoginIsSuccessful() {
        ...
        sessionPostResponse = ["sysSessionId": "the-session-id-98761", "user": ["sysObjectId": "the-user-id-41231", "sysUsername": "theusername2"]]
        
        var user: AXUser?
        var error: NSError?
        AXUser.login(provider: "google", fromViewController: nil) {
            user = $0; error = $1;
            async.fulfill()
        }
        ...
        waitForExpectationsWithTimeout(timeout + 1) { _ in
            ...
            AXAssertNotNil(user)
            AXAssertEqual(user?.username, "theusername2")
            AXAssertEqual(user?.objectID, "the-user-id-41231")
            AXAssertEqual(user, AXUser.currentUser())
            AXAssertEqual(Appstax.defaultContext.apiClient.sessionID, "the-session-id-98761")
            
            AXAssertEqual(self.keychain?.objectForKeyedSubscript("SessionID"), "the-session-id-98761")
            AXAssertEqual(self.keychain?.objectForKeyedSubscript("Username"), "theusername2")
            AXAssertEqual(self.keychain?.objectForKeyedSubscript("UserObjectID"), "the-user-id-41231")
        }
    }
